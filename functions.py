...
"""
목적: pre-trained network를 fine tuning하는 거야
근데 이게 결국 비디오 수준으로 보는 것이 아니야
비디오 폴더에 있는 이미지를 보고 classification하는 것이므로
image classification task로 볼 수 있어
따라서 비디오 폴더 경로를 주고 임의로 몇개의 data만 sample할 수 있으면 장떙
"""

"""
step1: 비디오 폴더들을 전부 받아
step2: 폴더에서 random으로 20개의 이미지를 sample하여 x, y로 구하기
step3: load하기
"""
class Dataset_FineTune(data.Dataset):
    def __init__(self, data_path, folders, labels, transform=None):
        """
        data_path: ucf_image경로
        folders: ucf_image경로 위에 있는 비디오 폴더들(이름) 
        labels: 비디오 폴더 이름을 통해 얻은 label을 숫자로 encoding한 array
        transform: image 변환        
        """
    ...
    def read_image(self, data_path, folder, use_transform):
        ...
        # 폴더에서 random으로 20개의 이미지를 sample
        num_of_frames_in_video = len(os.listdir(video_path))
        selected_frames = random.sample( range( 0, num_of_frames_in_video), 20)
        
        
        for i in selected_frames:
            path = os.path.join(video_path, '{}_{}.jpg'.format(folder, i+1))
            image = Image.open(path) # .convert('L'): color img -> gray img
    ...
    def __getitem__(self, index):
        ...
        x = self.read_image(self.data_path, folder, self.transform)
...
#             image = Image.open(os.path.join(path, selected_folder, '{}_{}.jpg'.format(selected_folder,i+1))).convert('L') 
            image = Image.open(os.path.join(path, selected_folder, '{}_{}.jpg'.format(selected_folder,i+1)))
            # .convert('L') -> color img에서 흑백 이미지로 바꾸는 기능
